...
public class ProductRepository
{
    ...
    {
        return await _context.Products.AsNoTracking().Include(x => x.Category).Include(x => x.Criterions).AsNoTracking()
            .ToListAsync();
    }
    ...
    {
        var category = await _context.Categories.Include(x => x.Criterions)
    ...
    {
        var product = await _context.Products.Include(x => x.Reviews).ThenInclude(x => x.CriterionRatings)
                          .FirstOrDefaultAsync(x => x.Name == name)
                      ?? throw new Exception("Данный продукт не найден");
